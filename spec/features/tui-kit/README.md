...
A `tea.MouseWheelMsg` MUST ALWAYS be broadcast to transcript `Block`s first (`transcript.Model.Update`, the same broadcast `dispatchUnhandled` gives every message chatshell does not itself fully own -- e.g. a grid reacting to it), regardless of which of the routing branches below additionally applies.

By default, and EXCEPT while an `Overlay` is on the stack (where it is already captured as overlay input, per REQ: chatshell-overlay's existing `isOverlayInputMsg` classification, unchanged by this REQ, and never reaches any of the branches below), a `tea.MouseWheelMsg` MUST scroll the transcript viewport (`tea.MouseWheelUp`/`tea.MouseWheelDown` calling `transcript.Model.ScrollUp`/`ScrollDown`) via chatshell's normal `Update` path. This is overridden when the pane is split (REQ: chatshell-side-panel's `splitEnabled`) and the event's `X` falls at or past the side/sidebar column (`chatWidth() + 3`, the width of the " │ " divider `View()` draws between the chat column and that column) -- there, the transcript does NOT scroll, and instead:

- when a `SidePanel` is installed (`WithSidePanel`), the raw event is forwarded to `SidePanel.Update` (free to interpret `X`/`Y`/`Button` itself);
